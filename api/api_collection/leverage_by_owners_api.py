...
class Api(BaseApi):
    ...
    # pool_owner 的delegation amount历史
        ...
        print(pool_pledge_amount_delegation.head())

        epoch_list = pool_pledge_amount_delegation['epoch_no'].drop_duplicates().values.tolist()  


        # def leverage (pool_pledge_amount_delegation,selected_epoch):
        #     selected_df = pool_pledge_amount_delegation[pool_pledge_amount_delegation["epoch_no"] ==selected_epoch ]
        #     selected_df["leverage"] =  pool_pledge_amount_delegation.apply(lambda y:  (y['pool_amount']-y['delegation_amount'])/y['pledge_amount']  if  ( y['pledge_amount'] != 0 ) else "No Pledge" ,  axis=1)
        #     return selected_df
