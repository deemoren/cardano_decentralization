...
class Api(BaseApi):
    ...
    def run(self, input) -> ApiOutput:
        ...
        def from_sql_to_df (sql_x):
            ...
            return x_df
        def total_ratio(selected_df, selected_epoch):
            selected_df = pool_pledge_amount_delegation[pool_pledge_amount_delegation["epoch_no"] ==selected_epoch ]
            total_pledge = selected_df["pledge_amount"].sum()
            total_amount = selected_df["pool_amount"].sum()
            x= total_pledge/total_amount
            return x, selected_epoch
    ...
    # pledge ratio by epoch   , epoeh 225 有问题
        ratio_df_all= []
        for selected_epoch in epoch_list:
            temp_tuple = total_ratio(pool_pledge_amount_delegation,selected_epoch)
            ratio_df_all.append(temp_tuple)

        ratio_df_all = pd.DataFrame(ratio_df_all)
        ratio_df_all.columns = ["pledge_ratio","epoch_no"]
        
        result = ratio_df_all.to_json(orient="records")
